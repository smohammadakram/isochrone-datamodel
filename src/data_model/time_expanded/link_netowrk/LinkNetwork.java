...
public class LinkNetwork {
	...
	private String city;
	private ArrayList<PGgeometry> bNodes;
	private HashMap<String, String> nearestEdge;
	private HashMap<String, PointLocation> pointLocation;
	private HashMap<Long, String> additionalNodes;
	...
	public void getBusNodes(){
		...
		ResultSet nodes = db.executeSimpleQuery("SELECT DISTINCT node_geometry AS n_geometry FROM " + TimeExpTablesDescription.SCHEMA_NAME + "." + city + "_bus_nodes");
	...
	public void getNearestStreetEdge(){
		...
		for(PGgeometry geom : bNodes){
			ResultSet rs = db.executeSimpleQuery("SELECT edge_geometry, "
	...
	public void getPointLocation(){
		...
		for(String s : nearestEdge.keySet()){
			ResultSet rs = db.executeSimpleQuery("SELECT edge_geometry, "
			...
			try {
				rs.first();
				PointLocation pl = new PointLocation(s, rs.getFloat("p_loc"), rs.getString("edge_geometry"));
	...
	public void getIntersecatedPoints(){
		...
		for(String s : pointLocation.keySet()){
			PointLocation pl = pointLocation.get(s);
//			ResultSet rs = db.executeSimpleQuery("SELECT ST_LineInterpolatePoint('" + pl.getEdgeGeom() + "', " + pl.getLocation() + ") AS geom "
			ResultSet rs = db.executeSimpleQuery("SELECT ST_LineInterpolatePoint(edge_geometry, " + pl.getLocation() + ") AS geom "
					+ "FROM " + TimeExpTablesDescription.SCHEMA_NAME + "." + city + "_street_edges "
			...
			try {
				...
				while(rs.next()){
					...
					PGgeometry geom = new PGgeometry(PGgeometry.geomFromString(rs.getString("geom")));
					//<key=node, value=edge>
					maxStreetID++;
					additionalNodes.put(maxStreetID, pl.getEdgeGeom());
					db.insertStreetNode(maxStreetID, geom.toString(), city);
					LinkEdge le;
					for(Integer i : db.getBusNodeByGeometry(s, city)){
						le = new LinkEdge(maxStreetID, 1, (long) i, 0);
						linkEdges.add(le);
						le = new LinkEdge((long) i, 0, maxStreetID, 1);
						linkEdges.add(le);
					}
				}
			} catch (SQLException e) {
	...
	public void builtInterBusLinks(){
		...
		try{
			ResultSet rs = db.executeSimpleQuery("SELECT bn1.node_id AS id_1, bn2.node_id AS id_2 "
			...
			rs.beforeFirst();
			while(rs.next()){
				LinkEdge le = new LinkEdge((long) rs.getInt("id_1"), 0, (long) rs.getInt("id_2"), 0);
				linkEdges.add(le);
				le = new LinkEdge((long) rs.getInt("id_2"), 0, (long) rs.getInt("id_1"), 0);
				linkEdges.add(le);
			}
		} catch (SQLException e){
			e.printStackTrace();
...
//	}
	...
	public void updateStreetEdges(){
		...
		try {
			for(Long l : additionalNodes.keySet()){
				ResultSet rs = db.executeSimpleQuery("SELECT edge_id, edge_source, edge_destination FROM " + TimeExpTablesDescription.SCHEMA_NAME + "." + city + "_street_edges "
						+ "WHERE edge_geometry = '" + additionalNodes.get(l) + "'");
				
				rs.beforeFirst();
				while(rs.next()){
					String geom = db.getGeometryByNodeID(l, city);
...
//			}
		...
		try{
			...
			while(rs.next()){
				if(inDegree.get(rs.getLong("node_id")) != null){
					int oldDegree = inDegree.get(rs.getLong("node_id"));
			...
			while(rs.next()){
				if(outDegree.get(rs.getLong("node_id")) != null){
					int oldDegree = outDegree.get(rs.getLong("node_id"));
					outDegree.replace(rs.getLong("node_id"), rs.getInt("out_d") + oldDegree);
				}
				else
					outDegree.put(rs.getLong("node_id"), rs.getInt("out_d"));
			}
			for(Long l : inDegree.keySet())
	...
	public void updateBusNodes(){
		...
		try{
			...
			while(rs.next()){
				if(inDegree.get(rs.getLong("node_id")) != null){
					int oldDegree = inDegree.get(rs.getInt("node_id"));
			...
			while(rs.next()){
				if(outDegree.get(rs.getLong("node_id")) != null){
					int oldDegree = outDegree.get(rs.getInt("node_id"));
		...
		for(Integer i : outDegree.keySet())
